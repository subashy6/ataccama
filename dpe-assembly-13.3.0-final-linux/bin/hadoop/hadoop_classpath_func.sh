...
function findSparkHome {
	if [ -d "/opt/cloudera/parcels/SPARK2/lib/spark2/" ]; then
		...
		echo "SPARK_HOME IS NOT FOUND. Please Specify env.SPARK_HOME manually in executor spark properties."
		exit 1
    fi
}
...
function validateSparkProperties {
	...
	if ! [ "$found_history_address" = 1 ]; then
			echo "spark.yarn.historyServer.address variable is not set in executor properties."
			echo "Please specify the URL for YARN History Server."
			echo "Exiting..."
			exit 1
	fi
	if ! [ "$found_event_log" = 1 ]; then
			echo "spark.eventLog.dir variable is not set in executor properties."
			echo "Please specify the location of Spark logs."
			echo "Exiting..."
			exit 1
	fi
	if ! [ "$found_hive_jars" = 1 ]; then
			echo "spark.sql.hive.metastore.jars variable is not set in executor properties."
