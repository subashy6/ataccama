...
CLASS=$2
PROPF=$3
LAUNCH=${4:-com.ataccama.dqc.hadoop.launch.LaunchModelDB}
shift
shift
shift
shift
...
export CLASSPATH=$MYLIB:$LOCAL_CP:$CLASSPATH

[ "$DEBUG" -gt 0 ] && DEBUG_OPT=-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=$DEBUG

# this variable (DBR|DATABRICKS) informs runner class that it actually have to run its databricks version
# deprecated 
export SPARK_TYPE=DBR
